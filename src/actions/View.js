...
export const fetchView = () => async dispatch => {
    try {
        ...
        if (res.data) {
            dispatch({ type: FETCH_VIEWS, payload: res.data.data })
        } else {
            //Alert message
        }
    }
...
export const putView = (data,id) => async dispatch => {
    try {
        const res = await api.put(`view/${id}`, data);
        if (res.data.status=="success") {
                const dep=await api.get(`view`)
                if(dep.data.status=="success"){
                dispatch({ type: FETCH_VIEWS, payload: dep.data.data })
...
export const postView = (data) => async dispatch => {
    try {
        const res = await api.post(`view`, data)
        if (res.data.status=="success") {
            const disres = await api.get('view')
            if (disres.data.status=='success') {
                dispatch({ type: FETCH_VIEWS, payload: disres.data.data })
...
export const deleteView = (id) => async dispatch => {
    try {
        const res = await api.delete(`view/${id}`)
        if (res.data.status=='success') {
            const disres = await  api.get('view')
            if (disres.data.status=='success') {
                dispatch({ type: FETCH_VIEWS, payload: disres.data.data })
