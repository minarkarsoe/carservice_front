...
class ScrollTable extends React.Component {
  constructor(props) {
    ...
    this.columns = props.columns.concat({
      ...
      render: record => (
        <>
          <Link style={{ color: '#33ff33', marginLeft: '10px' }} to={"/module/view/" + record.id}>
            view
          </Link>{' '}
          <Link style={{ color: '#1890FF', marginLeft: '10px' }} to={"/module/edit/" + record.id}>
            edit
          </Link>{' '}
          <Popconfirm
          ...
          >
            <a style={{ color: '#ff3333', marginLeft: '10px' }}>delete</a>
          </Popconfirm>
        </>
      )
    });
    ...
    return (
      <Card bordered={false}>
       <Button type="primary" style={{ height: '100%', width: '200px', height: '50px', backgroundColor: '#4672bb' }}>
          <Link style={{ color: '#fff' }} to={'/module/create'}>
            Create New Module
          </Link>
        </Button>
        ...
        <h2 style={{ paddingBottom: '10px' }}>
          ...
          <Input
            ...
            placeholder="Search"
            prefix={<Icon type="search" style={{ color: 'rgba(0,0,0,.25)', marginRight: '5px' }} />}
          />
        </h2>
